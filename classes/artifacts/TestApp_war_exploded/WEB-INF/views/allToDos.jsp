...
<body>
	...
	<p>
		...
		&nbsp
		<a href="<c:url value='/listdone' />">Выполненные</a>
		&nbsp
		<a href="<c:url value='/listactual' />">Не выполненные</a>
	</p>
	<table border="1">
		<tr>
			<td>Id</td>
		...
		<c:forEach items="${toDos}" var="toDo">
			<tr class="done_${toDo.done}">
                ...
                <td>
					<c:if test="${!toDo.done}">
						<a href="<c:url value='/edit-${toDo.id}-todo' />">Редактировать</a>
					</c:if>
				</td>
                ...
                <td>
					<c:choose>
						<c:when test="${!toDo.done}">
							<a href="<c:url value='/done-${toDo.id}-todo' />">Закончить</a>
						</c:when>
						<c:otherwise>
							Дело сделано
						</c:otherwise>
